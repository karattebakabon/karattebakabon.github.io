...
FJShowHatenaBookmarkInfo.show = function(data) {
    ...
    if (!data) {
        elm.innerHTML = '<p>�܂���񂪂���܂���B</p>';
        return;
    }
    var html = '<p>�͂Ăȃu�b�N�}�[�N�� : <a href="' + data.entry_url + '">' + data.count + '</a></p>';
    if (data.count > 0) {
        html += '<ul>';
        for (var i = 0, j = data.bookmarks.length; i < j; i++) {
            ...
            if (bookmark.comment) {
                html += '<br /><span class="bookmark-comment-title">�R�����g : </span>';
                html += '<span class="bookmark-comment-text">' +  bookmark.comment + '</span>';
            }
            if (bookmark.tags.length) {
                html += '<br /><span class="bookmark-tag-title">�^�O : </span>';
